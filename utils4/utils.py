...
def letterbox(combination, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    ...
    if shape[::-1] != new_unpad:  # resize
        ...
        lane = cv2.resize(lane, new_unpad, interpolation=cv2.INTER_LINEAR)
    ...
    lane = cv2.copyMakeBorder(lane, top, bottom, left, right, cv2.BORDER_CONSTANT, value=0)  # add border

    combination = (img, seg, lane)
...
def random_perspective(combination, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            lane = cv2.warpPerspective(lane, M, dsize=(width, height), borderValue=0, flags=cv2.INTER_NEAREST)
        else:  # affine
            im = cv2.warpAffine(im, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            seg = cv2.warpAffine(seg, M[:2], dsize=(width, height), borderValue=0, flags=cv2.INTER_NEAREST)
            lane = cv2.warpAffine(lane, M[:2], dsize=(width, height), borderValue=0, flags=cv2.INTER_NEAREST)
    # Visualize
